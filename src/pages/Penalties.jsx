...
export default function Penalties({ user }) {
  ...
  // Group leaderboard: km run per member
  const kmRunByUser = {}
  for (const r of allRuns) {
    kmRunByUser[r.user_id] = (kmRunByUser[r.user_id] ?? 0) + r.km_run
  }
  const kmOwedByUser = {}
  for (const p of penalties) {
    if (p.triggered_by) {
      kmOwedByUser[p.triggered_by] = (kmOwedByUser[p.triggered_by] ?? 0) + p.km_owed
    }
  }
      ...
      <div className="px-4 pt-4 flex flex-col gap-4">
        ...
        {done.length > 0 && (
          <div>
            ...
            <div className="flex flex-col gap-2">
              {done.map(p => (
                <PenaltyCard key={p.id} penalty={p} ran />
              ))}
            </div>
          </div>
        )}
          ...
          <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
            ...
            {members.map((m, i) => {
              ...
              return (
                <div
                  key={m.id}
                  className={`px-4 py-3 flex items-center gap-3 ${i < members.length - 1 ? 'border-b border-gray-50' : ''}`}
                >
                  <div
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="w-full h-1.5 bg-gray-100 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{ width: `${pct}%`, backgroundColor: left === 0 ? '#22c55e' : m.avatar_color }}
                      />
                    </div>
                  </div>
                  {left === 0 && owed > 0 && (
                  ...
                  {left > 0 && (
                    <span className="text-red-400 text-xs font-bold flex-shrink-0">{left}k</span>
                  )}
                </div>
              )
            })}
