export default function PenaltyCard({ penalty, ran, onRun }) {
  ...
  return (
    <div className={`bg-surface rounded-2xl p-4 shadow-sm flex items-center gap-3 ${ran ? 'opacity-50' : ''}`}>
      <div
      ...
      <div className="flex-1 min-w-0">
        <p className={`text-sm font-semibold text-ink ${ran ? 'line-through' : ''}`}>
          {penalty.users?.name ?? '—'}
        </p>
        <p className="text-xs text-ink-2 capitalize">{dateStr}</p>
      </div>
          ...
          <>
            ...
            {onRun && (
              <button
                ...
                className="text-xs bg-ink text-surface px-3 py-1.5 rounded-xl active:scale-95 transition-transform"
              >
                Couru
              </button>
            )}
