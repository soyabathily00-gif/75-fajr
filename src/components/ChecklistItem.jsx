export default function ChecklistItem({ rule, checked, weeklyCount, note, onToggle }) {
  ...
  function inverseBadgeStyle() {
    if (weeklyCount === 0) return 'bg-green-50 text-green-600 dark:bg-green-900/30 dark:text-green-400'
    if (weeklyCount < rule.target) return 'bg-amber-100 text-amber-600 dark:bg-amber-900/30 dark:text-amber-400'
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        {rule.id === 'R12' && note && (
          <span className="text-xs text-ink-3 truncate block mt-0.5 not-italic">{note}</span>
        )}
      </div>
      ...
      {rule.needsPhoto && !checked && (
        <svg className="w-4 h-4 text-ink-3 flex-shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
